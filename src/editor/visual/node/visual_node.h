...
public:
    explicit NodeWidget(const ::Node& data);
...
private:
    ...
    void buildStandardLayout(const ::Node& data);
    void buildVerticalToggleLayout(const ::Node& data);
    void buildPortRow(const std::string* left_name, PortType left_type,
                      const std::string* right_name, PortType right_type);
    void buildPortInColumn(Widget* col, const std::string& name, PortType type, bool is_left);
};
