...
void Port::render(IDrawList* dl, const RenderContext& ctx) const {
    ...
    float const r = PortConstants::RADIUS * ctx.zoom;
    Pt const center(pos.x + r, pos.y + r);
    dl->add_circle_filled(center, r, color(), 8);

    if (direction_ == bp2::Direction::InOut) return;
    float const thickness = PortConstants::ARROW_THICKNESS * ctx.zoom;
...
void Port::renderDebugPaintBounds(IDrawList* dl, const RenderContext& ctx) const {
    ...
    auto arrow = compute_port_arrow(layout_side_, direction_, center, r,
                                    arrow_offset, arrow_size);
    float const min_x = std::min({arrow.tip.x, arrow.back1.x, arrow.back2.x});
    float const min_y = std::min({arrow.tip.y, arrow.back1.y, arrow.back2.y});
    float const max_x = std::max({arrow.tip.x, arrow.back1.x, arrow.back2.x});
    float const max_y = std::max({arrow.tip.y, arrow.back1.y, arrow.back2.y});
