...
void HydraulicAccumulator<Provider>::execute(SimulationState& st, double dt) {
    float const p_in = st.values[provider.get(PortNames::p_in)];
    
    // Boyle's law: P_precharge * V_total = P_gas * V_gas
    // Gas pressure at current gas_volume:
    float const p_gas = precharge_pressure * volume / static_cast<float>(std::max(gas_volume, 0.01));
    ...
    float const dp = p_in - p_gas;
    float const flow_rate = dp * 0.001f;
