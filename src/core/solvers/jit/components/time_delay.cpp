...
void TimeDelay<Provider>::execute(SimulationState& st, double dt) {
    ...
    float const init_current_out = current_out + (raw_in - current_out) * first_frame_mask;
    float const init_last_in = last_in + (raw_in - last_in) * first_frame_mask;
    float const init_first_frame_mask = 0.0f;

    // 1b. Reset logic: if input changed from committed last_in, zero the timer
    float const new_accumulator = (raw_in == init_last_in) ? (accumulator + dt) : 0.0f;
    float const new_last_in = raw_in;

    // 1c. Select time threshold (delay_on if targeting 1, delay_off if targeting 0)
    float const target_delay = (raw_in > 0.5f) ? delay_on : delay_off;

    // 1d. Check timer expiration and state difference
    bool const timer_expired = (new_accumulator >= target_delay);
    bool const state_differs = (raw_in != init_current_out);

    // 1e. Compute next output only if timer expired
    float const new_current_out = (timer_expired && state_differs) ? raw_in : init_current_out;
