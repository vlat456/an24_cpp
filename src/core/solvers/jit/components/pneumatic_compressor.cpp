...
void PneumaticCompressor<Provider>::execute(SimulationState& st, double /*dt*/) {
    float const rpm = st.values[provider.get(PortNames::rpm_in)];

    // Centrifugal compressor characteristic: P ∝ RPM²
    // Normalized: P_out = max_pressure * (rpm / rated_rpm)²
    float const rpm_frac = std::clamp(rpm / rated_rpm, 0.0f, 1.5f);  // allow some overspeed
    float const pressure = max_pressure * rpm_frac * rpm_frac;
