...
void Spring<Provider>::execute(SimulationState& st, double dt) {
    float const pA = st.values[provider.get(PortNames::pos_a)];
    float const pB = st.values[provider.get(PortNames::pos_b)];

    // 1. Calculate current deformation
    float const delta_x = (pA - pB) - rest_length;
    ...
    float const spring_force = delta_x * k;
    ...
    float const inv_dt = 1.0f / static_cast<float>(std::max(dt, 1e-6));
    float const velocity = (delta_x - prev_delta_x) * inv_dt;
    float const damping_force = c * velocity;

    // 5. Total force = spring + damping (both resist motion)
    float const total_force = spring_force + damping_force;
    ...
    float const co_f = static_cast<float>(compression_only); // branchless bool→float (0.0 or 1.0)
    float const comp_mask = (delta_x < 0.0f) ? 1.0f : 0.0f;
    float const compression_mask = comp_mask * co_f + (1.0f - co_f);
