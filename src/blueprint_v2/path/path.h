...
public:
    ...
    core::InternedId segment() const { return segment_; }
...
private:
    ...
    Path(PathKind k, core::InternedId seg, uint32_t parent)
        : kind_(k), segment_(seg), parent_idx_(parent) {}

    PathKind kind_;
    core::InternedId segment_;
    uint32_t parent_idx_;
};

class PathArena {
public:
    explicit PathArena(core::StringInterner& interner)
        : interner_(interner) {
        paths_.push_back(Path(PathKind::Root, core::InternedId{}, 0));
    }

    Path root() const { return paths_[0]; }

    Path make_node(Path parent, core::InternedId node_id) {
        return add_path(PathKind::Node, node_id, parent);
    }

    Path make_port(Path parent, core::InternedId port_name) {
        return add_path(PathKind::Port, port_name, parent);
    }

    Path make_nested(Path parent, core::InternedId instance_id) {
        return add_path(PathKind::Nested, instance_id, parent);
    }

    Path make_wire(Path parent, core::InternedId wire_id) {
    ...
    std::string_view resolve_id(core::InternedId id) const { return interner_.resolve(id); }
    core::StringInterner& interner() const { return interner_; }

private:
    core::StringInterner& interner_;
    std::vector<Path> paths_;

    Path add_path(PathKind kind, core::InternedId seg, Path parent) {
...
struct std::hash<bp2::WireEndpoint> {
    size_t operator()(bp2::WireEndpoint const& ep) const noexcept {
        size_t h = std::hash<core::InternedId>{}(ep.node);
        h ^= std::hash<core::InternedId>{}(ep.port) + 0x9e3779b9 + (h << 6) + (h >> 2);
        return h;
    }
};
...
struct std::hash<bp2::Path> {
    size_t operator()(bp2::Path p) const noexcept {
        ...
        h ^= std::hash<core::InternedId>{}(p.segment()) + 0x9e3779b9 + (h << 6) + (h >> 2);
