...
TEST_F(CommandTest, ExtractToBlueprint_BasicAtomic) {
    ...
    EXPECT_EQ(in_bridge->structure.owner_scope, nested_sid);
    EXPECT_EQ(out_bridge->structure.owner_scope, nested_sid);
}
...
TEST_F(CommandTest, ExtractToBlueprint_AllowsSubgroupExtraction) {
    ...
    auto updated = editor::commands::build_extracted_blueprint_atomic(
        ...
        {interner.intern("a"), interner.intern("b")},
        "extracted_blueprint_1",
        WindowScopeId::embedded("group_1"),
        interner,
        arena,
        parser_registry,
        &err,
    ...
    EXPECT_EQ(collapsed->structure.owner_scope, "group_1");
...
TEST_F(CommandTest, ExtractToBlueprint_AllowsEmbeddedNestedInstanceSelection) {
    ...
    EXPECT_EQ(selected_nested_node->structure.owner_scope, new_nested_sid);
...
TEST_F(CommandTest, ManualBridgeAddition_SyncsCollapsedNodeAndNested) {
    ...
    const PortType new_port_type = PortType::Bool;
     ...
     bridge.structure.owner_scope = nested_id_str;
    ...
    set_iface(bridge, {
     ...
     bp2::Blueprint bp = updated->with_node(std::move(bridge));
      ...
      // Update nested iface only, then derive collapsed interface from nested authority.
      {
         ...
         const ui::InternedId iface_iid = interner.intern(iface_name);
          ports.push_back({iface_iid, Domain::Logical, bp2::Direction::Input, new_port_type});
      ...
      bp = bp2::sync_collapsed_node_iface_from_nested(bp, nested_id);
      ASSERT_TRUE(bp2::composite_iface_matches_nested(bp, nested_id));
    ...
    ASSERT_NE(bridge_node, nullptr) << "bridge node not found";
     EXPECT_EQ(bridge_node->structure.owner_scope, nested_id_str);
    ...
    ASSERT_NE(collapsed_post, nullptr);
     ...
     for (const auto& p : collapsed_post->semantic.iface.ports()) {
    ...
    set_iface(bridge, {
     ...
     bp2::Blueprint bp = updated->with_node(std::move(bridge));
      ...
      // Update nested iface only, then derive collapsed interface from nested authority.
      {
         ...
         std::vector<bp2::PortDescriptor> ports = n.inline_def()->iface().ports();
         const ui::InternedId iface_iid = interner.intern(iface_name);
          ports.push_back({iface_iid, Domain::Thermal, bp2::Direction::Output, new_port_type});
          *n.inline_def_mut() = n.inline_def()->with_interface(bp2::Interface(std::move(ports)));
          bp = bp2::replace_nested_preserve_order(bp, std::move(n));
      }
      bp = bp2::sync_collapsed_node_iface_from_nested(bp, nested_id);
